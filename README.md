...
dataset is small (510 leads) and this keeps the whole thing deployable as a
static-friendly Next.js app with zero infra.
...
example insight — _"Highway branch is 40% behind target
with 10 days left"_ — actually demonstrable, the insights engine anchors
...
only the pace insight uses this fixed anchor, since "days remaining in the
month" is meaningless for a month you're just browsing historically.
...
**No backend, no auth, no database** — the project is intentionally
self-contained. Data is imported
...
- **Mobile is "responsive," not "mobile-first."** The layout is optimized for
...
- **What-if scenarios** — a slider on test-drive→order conversion rate
  that recomputes projected revenue.
